...
error_while_loading_language = Error while loading language for locale: {$error}
fmt_preprocessor_command_could_not = preprocessor command could not start: {$cmd}: {$err}
fmt_preprocessor_command_failed = preprocessor command failed: {$cmd}: {$err}
...
# Auto-generated translations
error_separator_must_be_valid_1 = separator must be valid UTF-8 (use escape sequences \
                 to provide a separator that is not valid UTF-8)
error_sorting_by_last_modified_1 = sorting by last modified isn't supported: {err}
error_sorting_by_last_accessed_1 = sorting by last accessed isn't supported: {err}
error_sorting_by_creation_time_1 = sorting by creation time isn't supported: {err}
error_choice_unk_is_unrecognized_1 = choice '{unk}' is unrecognized
error_unrecognized_regex_engine_string_1 = unrecognized regex engine '{string}'
error_value_is_not_valid_1 = value is not valid UTF-8
error_size_is_too_big_1 = size is too big
error_unrecognized_flag_name_1 = unrecognized flag -{name}
error_msg_1 = {msg}
error_error_while_loading_language_1 = Error while loading language for locale: {}
error_usage_simplegrep_pattern_path_1 = Usage: simplegrep <pattern> [<path> ...]
fmt_06s_1 = {:0.6}s
fmt_preprocessor_command_could_not_1 = preprocessor command could not start: '{:?}': {}
fmt_preprocessor_command_failed_1 = preprocessor command failed: '{:?}': {}
fmt_wsldistro_1 = wsl$/{distro}
fmt_msg_consider_enabling_pcre2_1 = {msg}

Consider enabling PCRE2 with the --pcre2 flag, which can handle backreferences
and look-around.
fmt_msg_consider_enabling_multiline_1 = {msg}

Consider enabling multiline mode with the --multiline flag (or -U for short).
When multiline mode is enabled, new line characters can be matched.
fmt_msg_consider_enabling_text_1 = {msg}

Consider enabling text mode with the --text flag (or -a for short). Otherwise,
binary detection is enabled and matching a NUL byte is impossible.
fmt_unrecognized_flag_name_1 = unrecognized flag --{name}
fmt_msgnnsuggestmsg_1 = {msg}\n\n{suggest_msg}
fmt_missing_value_for_flag_1 = missing value for flag {mat}
fmt_error_parsing_flag_mat_1 = error parsing flag {mat}
fmt_name_1 = --{name}
fmt_similar_flags_that_are_1 = similar flags that are available: {list}
fmt_s_1 = -s {}
fmt_negated_1 = --{negated}
fmt_semver_rev_hash_1 = {semver} (rev {hash})
fmt_ripgrep_digits_1 = ripgrep {digits}
fmt_signsse2_1 = {sign}SSE2
fmt_signssse3_1 = {sign}SSSE3
fmt_signavx2_1 = {sign}AVX2
fmt_signneon_1 = {sign}NEON
fmt_signpcre2_1 = {sign}pcre2
fmt_itemlabel_1 = {item_label}
fmt_pieces_1 = {pieces}
fmt_line_1 = {line}:
error_while_loading_fallback_1 = Error while loading fallback language
show_num_lines_after_1 = Show NUM lines after each match.
show_num_lines_before_1 = Show NUM lines before each match.
search_binary_files_1 = Search binary files.
force_block_buffering_1 = Force block buffering.
print_the_byte_offset_1 = Print the byte offset for each matching line.
search_case_sensitively_default_1 = Search case sensitively (default).
when_to_use_color_1 = When to use color.
configure_color_settings_and_1 = Configure color settings and styles.
show_column_numbers_1 = Show column numbers.
set_the_separator_for_1 = Set the separator for contextual chunks.
show_count_of_matching_1 = Show count of matching lines for each file.
show_count_of_every_1 = Show count of every match for each file.
use_crlf_line_terminators_1 = Use CRLF line terminators (nice for Windows).
show_debug_messages_1 = Show debug messages.
numsuffix_1 = NUM+SUFFIX?
the_upper_size_limit_1 = The upper size limit of the regex DFA.
specify_the_text_encoding_1 = Specify the text encoding of files to search.
specify_which_regex_engine_1 = Specify which regex engine to use.
set_the_field_context_1 = Set the field context separator.
set_the_field_match_1 = Set the field match separator.
search_for_patterns_from_1 = Search for patterns from the given file.
print_each_file_that_1 = Print each file that would be searched.
print_the_paths_with_1 = Print the paths with at least one match.
print_the_paths_that_1 = Print the paths that contain zero matches.
treat_all_patterns_as_1 = Treat all patterns as literals.
follow_symbolic_links_1 = Follow symbolic links.
generate_man_pages_and_1 = Generate man pages and completion scripts.
include_or_exclude_file_1 = Include or exclude file paths.
process_all_glob_patterns_1 = Process all glob patterns case insensitively.
print_matches_grouped_by_1 = Print matches grouped by each file.
show_help_output_1 = Show help output.
search_hidden_files_and_1 = Search hidden files and directories.
run_a_program_to_1 = Run a program to get this system's hostname.
set_the_format_of_1 = Set the format of hyperlinks.
includeexclude_paths_case_insensitively_1 = Include/exclude paths case insensitively.
case_insensitive_search_1 = Case insensitive search.
specify_additional_ignore_files_1 = Specify additional ignore files.
process_ignore_files_case_1 = Process ignore files case insensitively.
include_zero_matches_in_1 = Include zero matches in summary output.
invert_matching_1 = Invert matching.
show_search_results_in_1 = Show search results in a JSON Lines format.
force_line_buffering_1 = Force line buffering.
show_line_numbers_1 = Show line numbers.
suppress_line_numbers_1 = Suppress line numbers.
show_matches_surrounded_by_1 = Show matches surrounded by line boundaries.
omit_lines_longer_than_1 = Omit lines longer than this limit.
show_preview_for_lines_1 = Show preview for lines exceeding the limit.
limit_the_number_of_1 = Limit the number of matching lines.
descend_at_most_num_1 = Descend at most NUM directories.
ignore_files_larger_than_1 = Ignore files larger than NUM in size.
search_with_memory_maps_1 = Search with memory maps when possible.
enable_searching_across_multiple_1 = Enable searching across multiple lines.
make_match_line_terminators_1 = Make '.' match line terminators.
never_read_configuration_files_1 = Never read configuration files.
dont_use_ignore_files_1 = Don't use ignore files.
dont_use_ignore_or_1 = Don't use .ignore or .rgignore files.
dont_use_local_exclusion_1 = Don't use local exclusion files.
dont_use_ignorefile_arguments_1 = Don't use --ignore-file arguments.
dont_use_global_ignore_1 = Don't use global ignore files.
suppress_gitignore_parse_error_1 = Suppress gitignore parse error messages.
suppress_some_error_messages_1 = Suppress some error messages.
use_gitignore_outside_of_1 = Use .gitignore outside of git repositories.
disable_unicode_mode_1 = Disable Unicode mode.
unicode_mode_1 = Unicode mode
print_a_nul_byte_1 = Print a NUL byte after file paths.
use_nul_as_a_1 = Use NUL as a line terminator.
skip_directories_on_other_1 = Skip directories on other file systems.
print_only_matched_parts_1 = Print only matched parts of a line.
set_the_path_separator_1 = Set the path separator for printing paths.
a_path_separator_must_1 = A path separator must be exactly one byte, but \
                 the given separator is {len} bytes: {sep}\n\
                 In some shells on Windows '/' is automatically \
                 expanded. Use '//' instead.
print_both_matching_and_1 = Print both matching and non-matching lines.
enable_pcre2_matching_1 = Enable PCRE2 matching.
print_the_version_of_1 = Print the version of PCRE2 that ripgrep uses.
search_output_of_command_1 = Search output of COMMAND for each PATH.
include_or_exclude_files_1 = Include or exclude files from a preprocessor.
alias_for_colors_headings_1 = Alias for colors, headings and line numbers.
do_not_print_anything_1 = Do not print anything to stdout.
the_size_limit_of_1 = The size limit of the compiled regex.
a_pattern_to_search_1 = A pattern to search for.
replace_matches_with_the_1 = Replace matches with the given text.
search_in_compressed_files_1 = Search in compressed files.
smart_case_search_1 = Smart case search.
sort_results_in_ascending_1 = Sort results in ascending order.
sort_results_in_descending_1 = Sort results in descending order.
print_statistics_about_the_1 = Print statistics about the search.
stop_searching_after_a_1 = Stop searching after a non-match.
search_binary_files_as_1 = Search binary files as if they were text.
set_the_approximate_number_1 = Set the approximate number of threads to use.
show_trace_messages_1 = Show trace messages.
trim_prefix_whitespace_from_1 = Trim prefix whitespace from matches.
only_search_files_matching_1 = Only search files matching TYPE.
add_a_new_glob_1 = Add a new glob for a file type.
clear_globs_for_a_1 = Clear globs for a file type.
do_not_search_files_1 = Do not search files matching TYPE.
show_all_supported_file_1 = Show all supported file types.
reduce_the_level_of_1 = Reduce the level of 
print_ripgreps_version_1 = Print ripgrep's version.
print_results_in_a_1 = Print results in a vim compatible format.
print_the_file_path_1 = Print the file path with each matching line.
never_print_the_path_1 = Never print the path with each matching line.
pcre2_is_not_available_1 = PCRE2 is not available in this build of ripgrep
this_flag_can_be_1 = This flag can be disabled with \fB\-\-{negated}\fP.
pcre2_is_available_1 = PCRE2 {}.{} is available
